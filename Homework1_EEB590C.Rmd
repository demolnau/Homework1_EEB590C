...
sum_pvalue_uncor_tests <- 0
count_pvalue_uncor_tests <- 0

for (i in 1:length(anova_lm_uncor_tests)) {
  if(anova_lm_uncor_tests[[i]][["Pr(>F)"]][1] > 0.05){
    count_pvalue_uncor_tests <- count_pvalue_uncor_tests + 1
  sum_pvalue_uncor_tests <- sum_pvalue_uncor_tests + anova_lm_uncor_tests[[i]][["Pr(>F)"]][1]
}
}

average_pvalue_uncor_tests <- sum_pvalue_uncor_tests/count_pvalue_uncor_tests
...
sum_pvalue_cor_tests <- 0
count_pvalue_cor_tests <- 0

for (i in 1:length(anova_lm_cor_tests)) {
    if(anova_lm_cor_tests[[i]][["Pr(>F)"]][1] > 0.05){
      count_pvalue_cor_tests <- count_pvalue_cor_tests + 1
  sum_pvalue_cor_tests <- sum_pvalue_cor_tests + anova_lm_cor_tests[[i]][["Pr(>F)"]][1]
    }
}

average_pvalue_cor_tests <- sum_pvalue_cor_tests/count_pvalue_cor_tests
#TODO -> sum up the pvalues that were less than 0.05
```
